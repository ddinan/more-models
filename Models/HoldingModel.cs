...
namespace MoreModels
{
    class HoldingModel : HumanoidModel
    {
        ...
        public override void RecalcProperties(Entity p)
        {
            ...
            if (scale >= 2f)
            {
                //Change the block that the player is holding if it is signaled that the block should change and signal that the block should no longer be changed if the scale changes
        ...
        {
            IModel blockModel = game.ModelCache.Get("block");
            if (blockModel != null)
            {
